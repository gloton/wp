<?php
/*
  Plugin Name: Chapter 2 - Page Header Output V6
  Plugin URI: 
  Description: Companion to recipe 'Adding output content to page headers using plugin actions'. Receta Processing and storing plugin configuration
data
  Author: ylefebvre
  Version: 1.0
...
<?php }
#admin_init
# cuando se loguea como administrador
add_action( 'admin_init', 'ch2pho_admin_init' );
function ch2pho_admin_init() {
	#1er parametro es un hook
	# es una mescla que comienza con la palabre admin_post_ y el valor del campo oculto <input type="hidden" name="action" value="save_ch2pho_options" />
	#2do parametro
	# es la llamada a una funcion
	add_action( 
		'admin_post_save_ch2pho_options',
...
function process_ch2pho_options() {
	...
	exit;
}//fin funcion process_ch2pho_options
?>